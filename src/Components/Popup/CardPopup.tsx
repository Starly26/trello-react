...
const CardPopup:React.FC <CardPopupProps>= ({name, close, columnName, userName, card, visible, changeDescription, comments, addComment, changeComment, changeName, removeComment, removeDescription}) => {
  ...
  return (
   <Container>
     <Wrapper>
       ...
       { isVisibleDesc ?
          <AreaWrapper>
          <TextAreaField id={card.id} placeName='Добавить более подробное описание' btnName='Сохранить' 
          close={() => setIsVisibleDesc(false)} change={closeDescription}/>
          </AreaWrapper>
        : 
          <div>
       ...
       <p>Автор {userName}</p>
       <AreaWrapper>
          <TextAreaField  id={card.id} placeName='Напишите комментарий' btnName='Сохранить' change={addComment}/>
       </AreaWrapper>
       {comments.map(comment => {
         if (comment.cardId === card.id) {
          return (
            <div key={comment.id}>
              <p>{userName}</p>
              <CommentField comment={comment} changeComment={changeComment} removeComment ={removeComment}/>
            </div>
          )
         }
       })} 
